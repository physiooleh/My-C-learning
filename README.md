...
#include <iostream> //operator wwodu-wywodu, daje nam cout, cin, endl
#include <fstream> //ifstream czytanie z pliku, ofstream zapisywanie w plik (ofstream file("data.txt"))
#include <string.h> //stara C-biblioteka(pracuje z char-masywami strlen, strcpy, strcmp), lepiej w nowoczesnym C++ <cstring>
#include <vector> //masyw dynamiczny czyj rozmiar moze sie zmieniac
#include <functional> //dla funkcji jako obiektow function <void()>f
#include <cmath> //matematyka  sqrt, pow, sin, 
#include <cstdlib> //rzeczy systemowe system(), rand(), exit() n.p. system("cls")
#include <ctime> //praca z czasem time(0);
#include "login.h" //plik podlaczony przez autora kodu np. class login {public: void() Login;}
using namespace std;

void login::Login() //login - User definied class, ::operator wydymosti objektu, ()spysok parametriw funkcji 
{
    string count;
